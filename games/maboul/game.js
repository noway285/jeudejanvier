...
async function saveScore(name, points) {
    ...
    try {
        ...
        console.log('✅ Score saved to Server API');
    } catch (error) {
        console.warn('⚠️ Could not save to Server API (Offline?):', error);
    }
...
async function loadLeaderboard() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Just basic info for leaderboard
            scores = data.map(s => ({
                name: s.name,
                points: s.points || 0
            }));
        } else {
            console.warn('⚠️ Server API error:', response.status);
            scores = state.localScores;
        }
    } catch (error) {
        console.warn('⚠️ Could not load from Server API (Offline?):', error);
        scores = state.localScores;
    }
...
async function loadAllScoresForAdmin() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            allScores.forEach(s => {
                // If loaded from JSON, re-parse dates or keep string
                if (!s.timestamp && s.date) s.timestamp = new Date(s.date).getTime();
            });
        } else {
            console.warn('⚠️ Server API error:', response.status);
            // Fallback: keep local scores if server fails
        }
    } catch (error) {
        console.warn('⚠️ Could not load from Server API (Offline?):', error);
    }
